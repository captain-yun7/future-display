...
export const DISPLAY_TYPES: DisplayTech[] = [
  ...
  },
  {
    ...
    principle: [
      { title: '백라이트(Backlight)', description: 'LCD는 스스로 빛을 내지 못하기 때문에, 뒤쪽에서 화면 전체를 밝히는 광원이 필요합니다' },
      { title: '하부 편광판(Bottom Polarizer)', description: '백라이트에서 나온 방향성이 없는 빛을 일정한 방향으로 정렬시킵니다' },
      { title: '액정층(Liquid Crystal Layer)', description: '하부 편광판과 상부 편광판 사이에 위치한 핵심 부분입니다. 전압이 걸리지 않으면 액정 분자가 비틀린 형태로 빛을 90도 회전시켜 통과시키고, 전압이 걸리면 분자가 정렬되어 빛이 회전하지 못해 막히게 됩니다. 이 원리를 이용해 전압의 세기에 따라 화면의 밝기를 조절합니다' },
      { title: '상부 편광판(Top Polarizer)', description: '하부 편광판과 서로 직각 방향으로 배치되어 있습니다. 전압이 없는 상태에서는 액정이 빛을 비틀어 통과시켜 화면이 밝게 보이고, 전압이 걸리면 빛이 통과하지 못해 어둡게 보입니다' },
      { title: 'TFT Array(박막 트랜지스터 어레이)', description: '각 픽셀마다 붙어 있는 미세한 전자 스위치로, 전압을 정밀하게 제어해 액정의 움직임을 조절합니다. 이것 덕분에 각 픽셀을 개별적으로 제어할 수 있습니다' },
      { title: '컬러 필터(Color Filter)', description: '컬러 필터를 통해 RGB 색상이 구현되어 다양한 색상을 표현할 수 있습니다' },
    ],
    manufacturing: [
  ...
  },
  {
    ...
    principle: [
      { title: '픽셀(Pixel) 구성', description: 'LED 디스플레이의 화면은 수많은 픽셀(Pixel)로 이루어져 있으며, 각 픽셀은 빨강(R), 초록(G), 파랑(B) 세 개의 미세한 LED 소자로 구성됩니다. 이 세 가지 색의 밝기를 조합하여 다양한 색상을 표현합니다. 예를 들어, R·G·B가 모두 켜지면 흰색, 모두 꺼지면 검정색이 나타납니다' },
      { title: '드라이버 IC(Driver IC)', description: '각 LED 픽셀에는 전류를 제어하는 드라이버 IC가 연결되어 있습니다. 드라이버 IC는 영상 신호를 받아 각 색상의 밝기를 정밀하게 조절하며, 빠르게 점멸시켜(수백~수천 회/초) 눈에 보이는 연속적인 영상을 만들어냅니다. 이때 LED는 매우 빠른 응답속도를 가져 잔상 없이 부드러운 화면 전환이 가능합니다' },
      { title: '모듈(Module) 구성', description: '픽셀들이 일정한 간격으로 배열된 기판을 모듈(Module)이라고 합니다. 하나의 모듈은 수백~수천 개의 픽셀로 구성되며, 여러 모듈을 조합하여 원하는 크기의 디스플레이를 만들 수 있습니다' },
      { title: '반도체 발광 원리', description: 'LED는 전류가 흐를 때 전자가 정공(+)과 만나며 에너지를 빛으로 방출하는 성질이 있어 각 픽셀 자체가 광원으로 작동합니다. 이로 인해 명암비가 높고 색 재현력이 우수하며, 어두운 장면에서도 선명한 화질을 표현할 수 있습니다' },
    ],
    manufacturing: [
  ...
  },
  {
    ...
    principle: [
      { title: '전극 구조 형성 (Anode / Cathode)', description: 'OLED는 두 전극 사이에 유기물층이 끼워진 샌드위치 구조로, 양극(Anode)은 정공(+)을, 음극(Cathode)은 전자(–)를 공급하여 전원이 인가되면 이들이 유기층 내부로 이동합니다' },
      { title: '전하 주입 및 이동 (Charge Injection & Transport)', description: '전자가 음극에서, 정공이 양극에서 각각 주입되어 전자수송층(ETL)과 정공수송층(HTL)을 따라 발광층으로 이동하면서 전류가 흐르고, 두 입자가 중앙의 발광층에서 만나게 됩니다' },
      { title: '재결합과 발광 (Recombination & Emission)', description: '발광층에서 전자와 정공이 재결합(Recombination)할 때 에너지 준위 차이(밴드갭)만큼의 에너지가 빛으로 방출됩니다. 이 원리를 전계 발광(Electroluminescence)이라 부릅니다' },
      { title: '색상 구현', description: '빛의 색은 사용된 유기물의 종류에 따라 달라지며, 적색(R), 녹색(G), 청색(B) 유기물층을 조합해 다양한 색상을 표현합니다' },
      { title: '픽셀 독립 제어', description: '각 픽셀이 독립적으로 빛을 내기 때문에 완벽한 검정색과 높은 명암비를 표현할 수 있으며, 얇고 유연한 구조로 만들 수 있어 휘어지는(Flexible), 접히는(Foldable), 말리는(Rollable) 형태의 디스플레이에도 적용됩니다' },
    ],
    manufacturing: [
  ...
  },
  {
    ...
    principle: [
      { title: '초소형 LED 픽셀 구성', description: 'Micro LED 디스플레이는 머리카락 굵기의 1/100 이하 크기로 줄인 초소형 LED(마이크로 LED)를 픽셀 단위로 배열합니다. 각 픽셀이 독립된 LED 소자로 구성되어 있으며, 각각의 소자가 스스로 빛과 색을 내기 때문에 백라이트나 컬러필터가 필요 없습니다' },
      { title: '무기물 기반 자발광', description: 'Micro LED는 OLED처럼 자발광하지만, 유기물이 아닌 무기물 LED 소자를 사용하기 때문에 번인(Burn-in) 현상이 없고 수명이 길며, 열과 산소, 수분에도 강해 내구성과 신뢰성이 매우 뛰어납니다' },
      { title: 'RGB 서브픽셀 배열', description: 'RGB LED 칩을 서브픽셀로 배열하여 다양한 색상을 구현합니다. 기존 LED보다 수백 배 작지만, 밝기와 효율은 오히려 높고, 명암비는 OLED 수준 이상입니다' },
      { title: 'Epi 웨이퍼 기반 제작', description: 'Micro LED는 Epi 웨이퍼(Epitaxial Wafer) 위에서 성장시킨 LED 칩을 픽셀 단위로 잘게 나누어 회로 기판 위에 옮겨 심는 방식으로 제작됩니다. 이러한 미세 LED 칩은 기존 LED보다 훨씬 높은 해상도 구현이 가능합니다' },
      { title: '독립적 픽셀 제어', description: '각 LED가 독립적으로 점등/소등되며, 전류 제어를 통해 밝기를 조절합니다. 이를 통해 완벽한 검정색과 무한대에 가까운 명암비를 구현할 수 있습니다' },
    ],
    manufacturing: [
